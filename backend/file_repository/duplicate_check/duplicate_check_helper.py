...
class DuplicateCheckHelper:
    ...
    def __init__(self) -> None:
        ...
        # 初始化视频检查器
        video_checker: VideoChecker = VideoChecker(
        ...
        # 初始化图片检查器
        image_checker: ImageChecker = ImageChecker(
    ...
    def get_all_results(self) -> List[DuplicateGroupDBModel]:
        """
        用途：汇总所有检查器的查重结果。
        返回值说明：
            List[DuplicateGroupDBModel]: 合并后的重复文件组列表。
        """
        all_results: List[DuplicateGroupDBModel] = []
        for checker in self.checkers:
